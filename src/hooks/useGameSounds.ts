...
export function useGameSounds() {
  // Função auxiliar para tocar qualquer áudio
  const playSound = useCallback((path: string) => {
    try {
      // Cria uma nova instância de áudio a cada chamada. Isso permite que sons se sobreponham
      const audio = new Audio(path);

      audio.play().catch((e) => {
        // Os navegadores bloqueiam autoplay se o usuário não tiver interagido com a página. O .catch evita que o console fique cheio de erros vermelhos caso isso aconteça.
        console.warn("A reprodução do áudio foi bloqueada ou falhou:", e);
      });
    } catch {
      // Ignora de forma segura caso o ambiente não suporte Audio
    }
  }, []);
